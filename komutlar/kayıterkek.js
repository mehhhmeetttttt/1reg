...
exports.run = async (client, message, args) => {
  ...
  const emoji = client.emojis.cache.find(emoji => emoji.name === "mavikelebek")
const emoji1 = client.emojis.cache.find(emoji => emoji.name === "mavikelebek")
const emoji2 = client.emojis.cache.find(emoji => emoji.name === "mavikelebek")
const emoji4 = client.emojis.cache.find(emoji => emoji.name === "mavikelebek")

 if (!message.member.roles.cache.has('787986332176875540') && !message.member.hasPermission('ADMINISTRATOR')) return message.channel.send(new Discord.MessageEmbed().addField(`${emoji}  Bilginize` , `${emoji1}  Bu komutu kullanmak için gerekli yetkiye sahip değilsin!`).setColor("BLACK").setFooter(message.author.tag ,message.author.avatarURL()).setTimestamp());
...
await rochelle.setNickname(`☆ ${isim} | ${yas}`)
  rochelle.roles.add("787986339794124840"); 
  rochelle.roles.add("787986340255760406");

  rochelle.roles.remove("787986337507573800"); // kadın 1
  rochelle.roles.remove("787986338305671198");

  rochelle.roles.remove("787986342889390090") // kayıtsız 1
  rochelle.roles.remove("787986333742268436") // cezalı 1
  

    message.react("777280401286234123");
db.add('yarteyit.'+message.author.id, 1)
  ...
  .setColor("BLACK")
  .setDescription(`${rochelle.user} kullanıcısı başarıyla erkek olarak kayıt edildi.`)                                                                     
  .setFooter(message.author.tag ,message.author.avatarURL())
  .setTimestamp()
